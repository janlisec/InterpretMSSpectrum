#' @title InterpretTP.
#'
#' @description \code{InterpretTP} is a wrapper function around \link{InterpretMSSpectrum} 
#'     which will read, evaluate and plot a deconvoluted mass spectrum (mass*intensity pairs)
#'     from either TMS-derivatized GC-APCI-MS data or ESI+/- data. 
#'     It allows to provide a chemical formula as a potential precursor of the spectrum.
#'     This formula will be used to set the parameters 'allowed_elements' and 
#'     'maxElements' during de-novo formula generation.
#'
#' @details For further details refer to \link{InterpretMSSpectrum}.
#'
#' @inheritDotParams InterpretMSSpectrum
#' @param fml A chemical formula of the standard used fot transormation product generation.
#'
...
#' InterpretTP(spec=apci_spectrum, correct_peak=cp, met_db=mdb, formula_db=fdb)
#'
#' @export
#'
#' @importFrom enviPat check_chemform mergeform

InterpretTP <- function(fml=NULL, ...) {
